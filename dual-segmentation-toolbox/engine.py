...
class Engine(object):
    def __init__(self, id=0, custom_parser=None):
        logger.info(
        ...
        if self.distributed:
            ...
            port = int(id) % 10
            if port == 0:
                dist.init_process_group(backend='nccl', init_method='file:///tmp/torch_dist_{}'.format(id),
                                        world_size=self.world_size, rank=self.local_rank)
            else:
                port += 21000
                dist.init_process_group(backend='nccl', init_method='tcp://127.0.0.1:{}'.format(port),
                                        world_size=self.world_size, rank=self.local_rank)
