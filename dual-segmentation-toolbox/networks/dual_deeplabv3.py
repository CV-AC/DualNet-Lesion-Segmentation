...
class ASPPModule(nn.Module):
    ...
    def __init__(self, features, inner_features=256, out_features=512, dilations=(12, 24, 36)):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.GroupNorm(num_channels=inner_features, num_groups=1))
...
class HeadBlock(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        self._simulate_make_layer(block, 512, 1, stride=1, dilation=4, multi_grid=(1, 1, 1))
...
class Dual_Deeplabv3(nn.Module):
    def __init__(self):
        super(Dual_Deeplabv3, self).__init__()
...
def Seg_Model(pretrained_model=None, logging_keys=True, load_default=False):
    model = Dual_Deeplabv3()

    if pretrained_model is not None:
        if load_default:
            model = load_model(model, pretrained_model, logging_keys=logging_keys)
        else:
